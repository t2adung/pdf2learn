...
Chỉ còn MỘT nguồn ảnh: trích ảnh gốc từ chính các trang PDF của topic (PyMuPDF)
-> AI vision lọc bỏ logo/trang trí, giữ ảnh có giá trị minh hoạ, kèm caption.
Chỉ chạy khi bật --book-images (mặc định TẮT).

Phần tóm tắt bài trước đây vẽ mindmap SVG nay chuyển thành BẢNG SO SÁNH thuần
markdown ngay trong cột content (render_markdown._comparison) — 0 token, hiển
thị mọi nơi, giúp học sinh nhớ lâu. Vì vậy stage này không còn sinh mindmap.
...
def generate_images_one(doc, row: dict, content_entry: dict, client, images_dir,
    ...
    book_images=False (MẶC ĐỊNH): KHÔNG trích ảnh trang sách. Phần tóm tắt bài
      đã được thể hiện bằng BẢNG SO SÁNH ngay trong content (0 token), nên topic
      không kèm ảnh cho gọn giao diện và tiết kiệm 1 request img_filter/topic.
    book_images=True: trích ảnh gốc từ trang PDF + AI vision lọc (hành vi cũ)."""
