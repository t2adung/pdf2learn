...
Chống lỗi chính tả trong ảnh (quan sát thực tế: model sinh ảnh viết sai
NHIỀU HƠN khi có nhiều chữ nhỏ/câu dài): CODE giới hạn MAX_BLOCKS khối nội
dung + số bullet/độ dài mỗi khối TRƯỚC khi đưa vào prompt — càng ít chữ,
model càng ít cơ hội viết sai. Đây là rút gọn chỉ cho ẢNH, không ảnh hưởng
nội dung markdown/JSON đầy đủ ở nơi khác.

QUAN TRỌNG: rút gọn bằng cách BỎ BỚT NGUYÊN CẢ bullet (không cắt giữa câu).
Cắt theo số từ (str.split()) không an toàn với tiếng Việt — nhiều "từ" ghép
2 âm tiết cách nhau bằng dấu cách (vd "lịch sử", "trọng tâm"), cắt giữa số
từ dễ đứt gãy giữa 1 từ ghép (vd "lịch sử" -> chỉ còn "lịch"). Prompt yêu
cầu model chép ĐÚNG NGUYÊN VĂN nên 1 bullet bị cắt cụt sẽ bị chép sai y hệt
phần cụt đó — thà bỏ hẳn bullet còn hơn cắt dở.
"""

COLOR_THEMES = ["blue", "green", "pink", "yellow", "purple"]
MAX_BLOCKS = 4              # khối nội dung chính (chưa tính khối "Ghi nhớ nhanh")
MAX_BULLETS_PER_BLOCK = 3
MAX_CHARS_PER_BLOCK = 140   # tổng độ dài bullet (ký tự) mỗi khối, đủ cho 2-3 câu ngắn
ASPECT_RATIO = "3:4"        # preset gần nhất với tỉ lệ trang A4 đứng (210x297mm ~ 1:1.414)


def _fit_bullets(bullets: list, max_count: int = MAX_BULLETS_PER_BLOCK,
                 max_chars: int = MAX_CHARS_PER_BLOCK) -> list:
    """Giữ NGUYÊN VẸN từng bullet — không cắt giữa câu. Bỏ bớt bullet CUỐI
    (giữ nguyên cả câu) nếu vượt max_count hoặc tổng ký tự vượt max_chars.
    Luôn giữ ít nhất 1 bullet đầu tiên dù nó dài hơn max_chars một mình."""
    out, total = [], 0
    for b in bullets:
        b = str(b).strip()
        if not b or len(out) >= max_count:
            break
        if out and total + len(b) > max_chars:
            break
        out.append(b)
        total += len(b)
    return out
...
def _poster_sections(lo: dict) -> list:
    """Learning Object -> list khối nội dung cho poster (đã rút gọn chữ,
    giới hạn MAX_BLOCKS khối): mỗi khối có heading/icon_hint/bullets (chưa
    gán number/color_theme, xem build_prompt)."""
    out = []
    ...
    for s in (lo.get("sections") or []):
        ...
        out.append({"heading": heading or "Nội dung", "icon_hint": icon, "bullets": bullets})
    ...
    out = out[:MAX_BLOCKS]   # ưu tiên giữ khối đầu (trọng tâm -> nội dung chính)
    for i, sec in enumerate(out):
        sec["number"] = i + 1
        sec["color_theme"] = COLOR_THEMES[i % len(COLOR_THEMES)]
        sec["bullets"] = _fit_bullets(sec["bullets"])
    return out
...
def build_prompt(lo: dict, title: str, subject_tag: str = "") -> str:
    ...
    quick_review = _fit_bullets([x for x in (lo.get("quick_review") or []) if str(x).strip()])
    ...
    return f"""Vẽ một tấm poster/infographic học tập khổ dọc, ĐÚNG TỈ LỆ TRANG A4 ĐỨNG
(210mm x 297mm, tỉ lệ rộng:cao xấp xỉ {ASPECT_RATIO}, ví dụ khung ảnh 1240x1754px) —
không vẽ vuông, không vẽ ngang. Phong cách hoạt hình dễ thương vẽ tay (hand-drawn
doodle), bảng màu pastel nhẹ nhàng (be, xanh mint, xanh dương nhạt, hồng phấn,
vàng nhạt), nền giấy kẻ ngang mờ màu kem.

BỐ CỤC (giữ đúng thứ tự từ trên xuống dưới, CHỈ {len(sections)} khối nội dung
chính + 1 khối chốt cuối — KHÔNG tự thêm khối nào khác):
...
2. Bên dưới là các khối nội dung, MỖI khối là 1 hình chữ nhật bo góc lớn, viền màu
   ...
   gạch đầu dòng NGẮN (dùng dấu chấm tròn nhỏ màu), và 1 hình minh hoạ doodle nhỏ đặt bên
...
YÊU CẦU CHỮ VIẾT (QUAN TRỌNG NHẤT — ưu tiên hơn mọi yêu cầu thẩm mĩ khác):
- Toàn bộ chữ trong ảnh là TIẾNG VIỆT CÓ DẤU, chép ĐÚNG NGUYÊN VĂN từng chữ cái
  và từng dấu thanh (sắc/huyền/hỏi/ngã/nặng) của nội dung đã cho ở trên — không
  được đổi, không được rút gọn, không được đoán mò khi không chắc.
- Nếu nội dung một khối quá dài không vừa khung, hãy VẼ KHUNG LỚN HƠN hoặc CỠ
  CHỮ NHỎ HƠN, TUYỆT ĐỐI không tự ý cắt/sửa/viết tắt sai từ.
- Dùng font chữ THẲNG, RÕ RÀNG, không chân (sans-serif) cho toàn bộ nội dung
  gạch đầu dòng và tiêu đề khối — CHỈ dòng tiêu đề lớn trên cùng mới được dùng
  kiểu chữ trang trí/viết tay. Chữ nhỏ dùng font trang trí rất dễ bị đọc/viết sai.
- Kiểm tra lại chính tả từng chữ một lần nữa trước khi hoàn thành ảnh.
