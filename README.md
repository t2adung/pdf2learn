...
| `--book-images` | Nhúng THÊM hình trích từ trang PDF — **0 token, thuần code**: định vị từng hình (`get_image_rects`) → render đúng vùng ra PNG (bắt cả nét vẽ/nhãn chồng lên), lấy **caption THẬT** của sách (dòng "Hình 1.2: …"), rồi **gán vào đúng mục nội dung bằng code** (so khớp caption + văn cảnh với heading/points). Mục "🖼️ Hình minh hoạ" cuối bài chỉ còn ảnh mindmap. Mặc định TẮT: chỉ giữ mindmap SVG (0 token) |
