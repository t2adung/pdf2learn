...
Làm 2 việc trên cột `content` (markdown):
  1. ẢNH: bỏ ảnh trang sách raster (.jpg/.jpeg/.png). Bài tóm tắt đã dùng bảng
     so sánh trong content nên phần ảnh thường trống — bước này chỉ có tác dụng
     khi bạn từng chạy --book-images.
...
def main():
    ...
    print(f"✅ {args.out}")
    print(f"   {len(rows)} topic | bỏ {img_removed} ảnh trang sách raster")
    if args.trim:
